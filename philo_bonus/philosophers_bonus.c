...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/13 15:12:46 by aserebry          #+#    #+#             */
/*   Updated: 2024/01/29 12:54:36 by aserebry         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	err_exit(t_base *base, char err_code)
{
	if (err_code == 'e')
		free(base);
	if (err_code == 'a')
		return (err_msg("You should use from 4 to 5 args!"));
	if (err_code == 'p')
		return (err_msg("Bad args!"));
	if (err_code == 'm')
		return (err_msg("Malloc err!"));
	return (1);
}
